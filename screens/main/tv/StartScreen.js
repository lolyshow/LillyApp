...
export default class StartScreen extends Component {
  ...
  plansList = () => {
    let plans = [{ label: "Select Tv Plan", value: "" }];
  ...
  getPackages = async (tv = "DSTV") => {
    try {
      ...
      if (response.packages || response.product) {
        ...
        Alert.alert("TV", "Could not fetch packages");
      }
    } catch (error) {
      this.setState({ loadingPackages: false });
  ...
  onchangePlan = (currentPlan) => {
    this.setState({ currentPlan, addonPLan: null, amount: null });
      ...
      <View style={styles.container}>
        ...
        >
              ...
              >
                ...
                >
                  <Image
                    source={item.image}
                    resizeMode="contain"
                    style={{ width: (screenWidth * 0.65) / 4, height: 70 }}
                  ></Image>
                </View>
              </TouchableWithoutFeedback>
            ))}
          ...
          <View style={styles.inputWrapper}>
            {["DSTV", "GOTV"].includes(this.state.selected) ? (
              <>
                {this.state.subscriptionType == "CHANGE_SUBSCRIPTION" && (
                  <MultiChoiceList
          ...
          {this.state.subscriptionType == "RENEW_SUBSCRIPTION" &&
            this.state.renewalAmountAvailable && (
              <>
                <View style={styles.inputWrapper}>
                  <InputBox
                    keyboardType="numeric"
                    onChangeText={this.onchangeMultiChoiceRenewalAmount}
                    inputValue={this.state.renewalAmount}
                    inputLabel="Amount Due / Amount to Pay"
                ...
                <View style={styles.inputWrapper}>
                  ...
                  >
                    Due Date For The Current Subscription.:{" "}
          ...
          {this.state.selected == "DSTV" &&
            this.state.subscriptionType == "BOX_OFFICE_SUBSCRIPTION" && (
              <View style={styles.inputWrapper}>
                <InputBox
                  keyboardType="numeric"
                  onChangeText={this.onchangeBoxOfficeAmount}
                  inputValue={this.state.box_office_amount}
                  inputLabel="Amount"
                  placeholder="Enter amount"
                />
              </View>
          ...
          <View style={styles.submitButtonWrapper}>
            ...
            !this.state.renewalAmountAvailable ? (
              <GreenButton
                text="Get Amount and Due Date"
                disabled={this.state.processing}
                processing={this.state.processing}
                onPress={() => this.renewValidation()}
              />
            ) : (
              <GreenButton
                text="Make Payment"
